...
// void rclBased(set<int> &X, set<int> &Y, list<int> *&adj, list<int> *&adjWeights, set<int> &U, int V, int alpha, double miu, int *sigmaX, int *sigmaY)
// {
//     set<int> rclSET;

//     // for each value of U set, find if it can be in RCL Set
//     for (auto v : U)
//     {
//         // calculate greedyValue
//         int greedyVal = max(sigmaX[v], sigmaY[v]);
//         if (greedyVal >= miu)
//         {
//             rclSET.insert(v);
//         }
...
//     while (!U.empty())
//     {
//         rclBased(X, Y, adj, adjWeights, U, V, alpha, miu, sigmaX, sigmaY);
//     }
//     ans = {X, Y};
//     return ans;
// }